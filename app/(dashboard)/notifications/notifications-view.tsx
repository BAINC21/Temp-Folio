...
const bgColor = (t: string) => t === "success" ? "bg-emerald-500/5" : t === "warning" ? "bg-amber-500/5" : t === "danger" ? "bg-red-500/5" : "bg-f-accent/5";
    ...
    <>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex gap-2.5">
          <button onClick={markAllRead} disabled={unreadCount === 0} className="px-4 py-2.5 rounded-lg text-sm font-semibold text-f-muted border border-f-border hover:border-f-muted hover:text-f-text transition-all disabled:opacity-40">
            Mark all read
          </button>
          <button onClick={clearAll} disabled={notifs.length === 0} className="px-4 py-2.5 rounded-lg text-sm font-semibold text-red-400 border border-red-400/30 hover:bg-red-400/10 transition-all disabled:opacity-40">
            Clear all
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-1.5 p-1 rounded-lg bg-f-surface border border-f-border w-fit mb-6">
        {(["all", "unread"] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-md text-xs font-semibold capitalize transition-all ${filter === f ? "bg-f-accent/15 text-f-accent-lt" : "text-f-muted hover:text-f-text"}`}>
      ...
      <div className="bg-f-surface border border-f-border rounded-xl overflow-hidden">
        {filtered.length === 0 && (
          <div className="px-6 py-12 text-center">
            <svg className="mx-auto mb-3 opacity-30" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#8888A0" strokeWidth="1.5"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0" /></svg>
        ...
        <div className="divide-y divide-f-border/50">
          {filtered.map(n => (
            <div key={n.id} onClick={() => markAsRead(n.id)}
              className={`px-5 py-4 flex gap-4 items-start cursor-pointer transition-colors hover:bg-f-hover ${!n.read ? bgColor(n.type) : ""}`}>
              <div className={`w-2.5 h-2.5 rounded-full mt-1.5 shrink-0 ${n.read ? "bg-[#3A3A48]" : dotColor(n.type)}`} />
              <div className="flex-1 min-w-0">
                <p className={`text-sm leading-relaxed ${n.read ? "text-f-muted" : "text-f-text font-medium"}`}>{n.text}</p>
