...
export function DatePicker({ value, onChange, placeholder = "Select date" }: {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute z-50 mt-2 w-72 bg-[#1C1C26] border border-f-border rounded-xl shadow-2xl p-4">
          ...
          <div className="grid grid-cols-7 gap-0.5 mb-1">
            {DAYS.map(d => <div key={d} className="text-center text-[10px] font-semibold text-f-muted py-1">{d}</div>)}
          </div>
          ...
          {value && (
            <button onClick={() => { onChange(""); setOpen(false); }} className="mt-2 w-full text-center text-xs text-f-muted hover:text-f-text transition-colors py-1">
              Clear date
            </button>
          )}
        </div>
